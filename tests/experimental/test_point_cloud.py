...
def test_units_the_panel_cannot_read_are_refused(tmp_path: Path, unit: str) -> None:
    """A unit `nominal.ts` knows but the 3D panel cannot read is refused, not converted.

    The extent is always stored in microseconds, but the panel re-reads the raw column
    values in a unit picked at view time and offers only nanoseconds through seconds.
    A 0..1 minute column would record a 0..60s extent that the panel then sweeps across
    raw 0..1 values -- wrong playback, with nothing to signal it.
    """
    with pytest.raises(ValueError, match="time_unit must be one of"):
        _describe(tmp_path, _TIMED_CSV, timestamp_column="t_s", time_unit=unit)
