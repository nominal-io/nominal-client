...
class TestRoutesAndRequestCounts:
    ...
    def test_small_route_makes_exactly_one_call(self, make_uploader: MakeUploader, write_file: WriteFile) -> None:
        """A file under the single-shot threshold costs exactly one API request."""
    ...
    def test_zero_byte_file_takes_multipart(self, make_uploader: MakeUploader, write_file: WriteFile) -> None:
        """Empty files always take multipart: the single-shot endpoint rejects a declared size of 0."""
        up, service, _ = make_uploader(small_file_route_max_bytes=1024)
...
class TestValidation:
    ...
    def test_multi_part_below_provider_minimum_rejected(
        self, make_uploader: MakeUploader, write_file: WriteFile
    ) -> None:
        """Non-final parts below the provider's 5 MiB minimum are rejected before any bytes are spent.

        The server would reject them at complete-time, after the whole file had uploaded.
        """
        up, _, _ = make_uploader()
...
class TestNonBlockingEnqueueAndCancellation:
    def test_enqueue_never_blocks_and_queued_files_do_not_initiate(
        ...
        up, service, session = make_uploader(service=service, max_storage_workers=2, max_files_in_flight=1)
...
class TestSharedUploadClient:
    def test_uses_the_clients_shared_upload_client(self) -> None:
        """The uploader rides the client's shared transport rather than building its own.

        The shared conjure client's transport-level throttle retries ARE the local layer of
        the two-layer backoff.
        """
...
class TestCreateValidation:
    ...
    def test_small_file_route_ceiling_is_four_mib(self) -> None:
        """The single-shot opt-in ceiling is a public contract; moving it is a breaking change."""
...
class TestSmallFileRoute:
    ...
    def test_a_file_above_the_threshold_still_takes_multipart(
        self, make_uploader: MakeUploader, write_file: WriteFile
    ) -> None:
        """Files bigger than the threshold ignore the single-shot route entirely."""
        up, service, _ = make_uploader(small_file_route_max_bytes=64)
        path = write_file("big.csv", 200)
    ...
    def test_passing_none_disables_the_route(self, make_uploader: MakeUploader, write_file: WriteFile) -> None:
        """`small_file_route_max_bytes=None` sends even tiny files through multipart."""
        up, service, _ = make_uploader(small_file_route_max_bytes=None)
        path = write_file("small.csv", 100)
