...
from nominal.core._grpc import api_base_url_to_grpc_target
...
def mock_dataset():
    ...
    clients.roles.get_resource_owner_rid.return_value = "ri.authn.user.owner"
    return dataset


def test_get_dataset_owner_rid_uses_role_service_client(mock_dataset: Dataset):
    assert get_dataset_owner_rid(mock_dataset) == "ri.authn.user.owner"
    mock_dataset._clients.roles.get_resource_owner_rid.assert_called_once_with("test-rid")  # type: ignore[attr-defined]
...
def test_get_dataset_owner_rid_raises_when_no_owner_found(monkeypatch: pytest.MonkeyPatch, mock_dataset: Dataset):
    mock_dataset._clients.roles.get_resource_owner_rid.return_value = None  # type: ignore[attr-defined]
