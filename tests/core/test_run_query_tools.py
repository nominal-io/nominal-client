...
def test_create_search_runs_query_with_asset_rids():
    """Test query creation with asset_rids filter."""
    asset_rids = ["ri.asset.main.asset.1", "ri.asset.main.asset.2"]
    query = create_search_runs_query(asset_rids=asset_rids)
...
def test_create_search_runs_query_with_empty_asset_rids():
    """Empty asset_rids should not add a filter."""
    query = create_search_runs_query(asset_rids=[])
    assert query.and_ == []
...
def test_create_search_runs_query_with_archived():
    """Test query creation with archived filter."""
    query_archived = create_search_runs_query(archived=True)
    assert query_archived.and_ is not None
    assert len(query_archived.and_) == 1
    assert query_archived.and_[0].archived is True

    query_active = create_search_runs_query(archived=False)
    assert query_active.and_ is not None
    assert query_active.and_[0].archived is False
