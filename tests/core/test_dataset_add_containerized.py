...
def _image_proto(required_input: str) -> reg_pb2.ContainerImage:
    return reg_pb2.ContainerImage(
        rid="ri.img.1",
        tag="v1",
        extractor_rid="ri.ext.1",
        status=reg_pb2.CONTAINER_IMAGE_STATUS_READY,
        file_output_format=reg_pb2.FILE_OUTPUT_FORMAT_PARQUET,
        inputs=[reg_pb2.FileExtractionInput(environment_variable=required_input, name=required_input, required=True)],
...
def _extractor(clients: MagicMock, *, active_image: reg_pb2.ContainerImage | None) -> ContainerizedExtractor:
    proto = ext_pb2.ContainerizedExtractor(rid="ri.ext.1", workspace_rid="ri.ws", name="parser", is_archived=False)
...
def test_add_containerized_validates_against_active_image_without_refetch() -> None:
    ...
    clients.registry.GetImage.return_value = reg_pb2.GetImageResponse(image=_image_proto("INPUT_FILE"))
...
def test_add_containerized_returns_ingestion_job() -> None:
    ...
    image = reg_pb2.ContainerImage(rid="ri.img.1", tag="v1", extractor_rid="ri.ext.1", file_output_format=1)
