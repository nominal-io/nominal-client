...
from nominal.core.point_cloud import _build_import_config, _describe_point_cloud_csv
...
def test_import_config_emits_the_v2_shape() -> None:
...
def test_xyz_are_geometry_whatever_their_case_and_are_not_attributes() -> None:
...
def test_archetype_is_ordered_by_bucket_not_by_header() -> None:
    ...
    config = _build_import_config(header, ["0,0,0,1.5,3,kerb,88.5,4"], {"ring": "int", "gear": "int"})
...
def test_each_column_type_declares_the_reductions_its_wire_type_allows(
    kind: str, wire_type: object, reductions: list[str]
) -> None:
    """Without reductions an attribute cannot drive ramp colouring or value-range filtering at all."""
    config = _build_import_config("x,y,z,col", ["0,0,0,1"], {"col": kind})  # type: ignore[dict-item]
...
def test_a_column_is_real_only_when_every_populated_sample_is_numeric(values: list[str], bucket: str) -> None:
    """One non-numeric value anywhere in the sample forces the whole column to string."""
    config = _build_import_config("x,y,z,col", [f"0,0,0,{v}" for v in values])

    assert config["format"]["columns"][bucket] == [3]


def test_integer_looking_columns_are_typed_real_not_int() -> None:
    ...
    """
    config = _build_import_config("x,y,z,count", ["0,0,0,1", "0,0,0,2"])
...
def test_column_types_override_wins_over_inference() -> None:
    ...
    config = _build_import_config("x,y,z,ring", ["0,0,0,1"], {"ring": "int"})

    assert config["format"]["columns"]["int"] == [3]
    assert config["format"]["columns"]["real"] == []


def test_column_types_override_cannot_reclassify_geometry() -> None:
...
def test_inference_reads_past_the_first_row(tmp_path: Path) -> None:
    ...
    assert described.import_config["format"]["columns"]["string"] == [3]
...
def test_rgb_column_becomes_a_single_rgb_attribute() -> None:
...
def test_without_rgb_column_a_hex_column_stays_a_string() -> None:
