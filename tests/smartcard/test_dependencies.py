...
def test_smartcard_dependency_check_passes_when_all_present(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert_required_dependencies_available() is None


def test_smartcard_dependency_check_names_missing_cffi(monkeypatch: pytest.MonkeyPatch) -> None:
    def find_spec(import_name: str):
        if import_name == "cffi":
            return None
        return object()
    ...
    with pytest.raises(SmartcardDependencyError, match="cffi"):
        assert_required_dependencies_available()


def test_smartcard_dependency_check_names_all_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("nominal.smartcard._dependencies.importlib.util.find_spec", lambda _: None)

    with pytest.raises(SmartcardDependencyError) as exc_info:
        assert_required_dependencies_available()

    error_message = str(exc_info.value)
    assert "PyKCS11" in error_message
    assert "cffi" in error_message
