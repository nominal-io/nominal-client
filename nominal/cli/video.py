...
@click.option("--gpu", type=click.Choice(["auto", "nvidia", "intel", "amd", "apple", "none"]),
              default="auto", show_default=True,
              help="GPU acceleration type to use")
@click.option("--preset", default="fast", show_default=True,
              help="Encoding preset (speed vs quality tradeoff)")
@click.option("--preserve-aspect/--stretch", default=True, show_default=True,
...
@click.option("--keyframe-interval", type=int, default=2, show_default=True,
              help="Interval between keyframes in seconds")
...
def convert_video(
    ...
    if target_resolution:
        if isinstance(target_resolution, str):
            ...
            click.echo(f"🎯 Target resolution: {target_resolution.resolution_width}x{target_resolution.resolution_height}")
...
def check_gpu() -> None:
    ...
    if available:
        ...
        click.echo(f"   nom video convert -i input.mp4 -o output.mp4 --gpu auto")
    else:
