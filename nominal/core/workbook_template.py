    ...
    def create_workbook(
        ...
        *,
        ...
        Args:
            ...
            run_rid: Run to visualize in the workbook
                NOTE: may not be provided alongside `asset_rids`
            asset_rids: Assets to visualize in the workbook
                NOTE: may not be provided alongside `run_rid`

        Returns:
            The instantiated workbook
        """
        if run_rid is not None and asset_rids is not None:
            raise ValueError("Only one of `run_rid` and `asset_rids` may be used to create a workbook from a template")
        elif run_rid is None and asset_rids is None:
            raise ValueError("One of `run_rid` or `asset_rids` must be provided to create a workbook from a template")
        ...
        request = scout_notebook_api.CreateNotebookRequest(
            ...
            data_scope=scout_notebook_api.NotebookDataScope(
                run_rids=None if run_rid is None else [run_rid],
                asset_rids=None if asset_rids is None else [*asset_rids],
            ),
