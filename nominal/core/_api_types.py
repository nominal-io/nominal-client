...
class _ApiContainerImage:
    ...
    @classmethod
    def _parse_list(cls, raw: Mapping[str, Any], key: str = "images") -> Sequence[_ApiContainerImage]:
        items = raw.get(key) or []
        if not isinstance(items, list):
            raise ValueError(f"unexpected list payload at {key!r}: {raw!r}")
        return [cls._parse(item) for item in items]
