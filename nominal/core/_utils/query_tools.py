...
def create_search_events_query(  # noqa: PLR0912
    ...
    if after is not None:
        queries.append(event_pb2.SearchQuery(after=_SecondsNanos.from_flexible(after).to_nominal_proto_timestamp()))
    if before is not None:
        queries.append(event_pb2.SearchQuery(before=_SecondsNanos.from_flexible(before).to_nominal_proto_timestamp()))
    if asset_rids:
