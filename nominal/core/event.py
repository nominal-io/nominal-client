...
class Event(HasRid, RefreshableGrpcMixin[event_pb2.Event]):
    ...
    def _get_latest_api(self) -> event_pb2.Event:
        return _get_event_proto(self._clients, self.rid)
    ...
    def update(
        ...
        *,
        ...
        updated_timestamp = None if start is None else _SecondsNanos.from_flexible(start).to_nominal_proto_timestamp()
    ...
    def _from_proto(cls, clients: _Clients, event: event_pb2.Event) -> Self:
        return cls(
            ...
            start=_SecondsNanos.from_nominal_proto_timestamp(event.timestamp).to_nanoseconds(),
            duration=_from_proto_duration(event.duration),
            type=EventType._from_proto(event.type),
            properties=dict(event.properties),
            labels=list(event.labels),
            created_by_rid=event.created_by if event.HasField("created_by") else None,
            _uuid=event.uuid,
            _clients=clients,
        )


def _batch_get_event_protos(clients: Event._Clients, rids: Sequence[str]) -> Sequence[event_pb2.Event]:
    with translate_grpc_errors():
        return clients.event.BatchGetEvents(event_pb2.BatchGetEventsRequest(event_rids=list(rids))).events


def _get_event_proto(clients: Event._Clients, rid: str) -> event_pb2.Event:
    ...
    """
    events = _batch_get_event_protos(clients, [rid])
    if not events:
...
def _create_event(
    ...
    *,
    ...
    request = event_pb2.CreateEventRequest(
        ...
        timestamp=_SecondsNanos.from_flexible(start).to_nominal_proto_timestamp(),
