...
class Asset(_DatasetWrapper, HasRid, RefreshableGrpcMixin[asset_pb2.Asset]):
    ...
    def _list_dataset_scopes(self) -> Sequence[DatasetScope]:
        return [
            ...
            for scope in _filter_proto_scopes(self._get_latest_api().data_scopes, "dataset")
        ]
    ...
    def update(
        ...
        *,
        ...
        updated_links = None if links is None else asset_pb2.LinkList(links=_create_proto_links(links))
...
def _create_proto_links(links: Sequence[str | Link | LinkDict]) -> list[asset_pb2.Link]:
