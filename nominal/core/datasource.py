...
class DataSource(HasRid):
    ...
    def add_channel(
        ...
        *,
        ...
        """
        spec = CreateChannelRequest(name=name, data_type=data_type, description=description, unit=unit)
        request = _build_series_metadata_request(self.rid, spec)
    ...
    def batch_add_channels(
        ...
        *,
        ...
        for batch in batched(channels, batch_size):
            requests = [_build_series_metadata_request(self.rid, spec) for spec in batch]
...
def _build_series_metadata_request(
    data_source_rid: str, spec: CreateChannelRequest
) -> timeseries_metadata_api.CreateSeriesMetadataRequest:
    nominal_data_type = spec.data_type._to_nominal_data_type()
