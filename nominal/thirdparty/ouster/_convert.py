...
def convert_ouster_dataset(
    ...
    *,
    ...
    Reads the data.yaml manifest, decodes each Ouster lidar PCAP using ouster-sdk, optionally applies
    navigation poses from buf3 nav data, and writes one CSV per sensor with columns:
    x, y, z, time, reflectivity, signal, near_ir.

    The resulting CSVs are in the shape that `Spatial.add_point_cloud_csv` ingests, so a converted
    dataset can be handed straight to a spatial created by `create_point_cloud_spatial`.
    ...
    Raises:
        ...
        ImportError: If ouster-sdk is not installed.
    """
    ...
    if apply_nav:
        ...
        if nav_path is not None:
            ...
            _, records = _parse_buf3(nav_path)
...
def _parse_buf3(path: Path) -> tuple[list[str], list[dict[str, float]]]:
    ...
    if data[:4] != b"buf3":
        raise ValueError(f"Not a buf3 file: {path}")
...
class _NavTrajectory:
    def __init__(self, records: list[dict[str, float]]) -> None:
        ...
        # Applying an empty trajectory would silently emit points in the wrong frame, so fail loudly
        # instead: the caller asked for nav correction and we have no poses to correct with.
        if not poses:
            raise ValueError("No usable nav poses found; pass apply_nav=False to import in the lidar frame")
