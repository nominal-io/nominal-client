...
The unit must be one of `nanoseconds`, `microseconds`, `milliseconds` or `seconds`. The 3D panel
re-reads the raw column values in a unit chosen at view time and offers only those four, so a coarser
or finer one is rejected rather than converted into a range nothing can line up against the values.
