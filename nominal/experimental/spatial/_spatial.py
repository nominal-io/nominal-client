        ...
        *accepted*, not when the point cloud is queryable. Poll the returned job to
        ...
        Args:
            ...
            timestamp_type: How to read ``timestamp_column``. Only `Relative` is accepted:
                ...
                at epoch magnitude is over two minutes. Its unit must be one the 3D panel
                can read per-point time in -- nanoseconds, microseconds, milliseconds or
                seconds -- since it re-reads the raw column values in a unit picked at
                view time and cannot name any other.
        ...
        Raises:
            ...
            TypeError: If ``timestamp_type`` is not a `Relative`.
        ...
        # part-way through duplicates every point -- so a bad pair has to fail while failing
        # is still free. The signature says `Relative` and means it; the isinstance check is
        # for callers the type checker never sees, who would otherwise get an AttributeError
        # off `.start` after the submission rather than a refusal before the upload.
        _validate_timestamp_pair(timestamp_column, timestamp_type)
        if timestamp_type is not None and not isinstance(timestamp_type, Relative):
            raise TypeError(
                f"timestamp_type must be a Relative, not {type(timestamp_type).__name__}: per-point time is an "
                "offset from a start instant, because filtering happens on the GPU in f32, where the spacing "
                "between representable values at epoch magnitude is over two minutes"
            )
